/*
    Fuente oficial de inferencia FINAN.
    No contiene target, número de tarjeta, CVV, domicilio ni coordenadas.
*/
SET NOCOUNT ON;
GO
...
(
    SELECT
        ...
        t.client_id,
        t.card_id,
        TRY_CONVERT(decimal(18, 2), REPLACE(REPLACE(t.amount, '$', ''), ',', '')) AS amount,
        t.use_chip,
        t.merchant_id,
        t.merchant_city,
        t.merchant_state,
        ...
        u.birth_year,
        u.birth_month,
        TRY_CONVERT(decimal(18, 2), REPLACE(REPLACE(u.per_capita_income, '$', ''), ',', '')) AS per_capita_income,
        TRY_CONVERT(decimal(18, 2), REPLACE(REPLACE(u.yearly_income, '$', ''), ',', '')) AS yearly_income,
        TRY_CONVERT(decimal(18, 2), REPLACE(REPLACE(u.total_debt, '$', ''), ',', '')) AS total_debt,
        u.credit_score,
        u.num_credit_cards,
        c.card_brand,
        c.card_type,
        ...
        c.num_cards_issued,
        TRY_CONVERT(decimal(18, 2), REPLACE(REPLACE(c.credit_limit, '$', ''), ',', '')) AS credit_limit,
...
SELECT
    ...
    ((DATEDIFF(day, CONVERT(date, '19000101'), CONVERT(date, transaction_date)) % 7) + 1) AS day_of_week,
    CONVERT(bit, CASE
        WHEN (DATEDIFF(day, CONVERT(date, '19000101'), CONVERT(date, transaction_date)) % 7) IN (5, 6)
    ...
    CASE
        ...
        ELSE DATEPART(year, transaction_date) - birth_year
             - CASE
                 WHEN birth_month IS NOT NULL AND DATEPART(month, transaction_date) < birth_month
                 THEN 1 ELSE 0 END
    ...
    account_open_year,
    CASE
        WHEN year_pin_last_changed IS NULL OR year_pin_last_changed > DATEPART(year, transaction_date)
        THEN NULL
        ELSE DATEPART(year, transaction_date) - year_pin_last_changed
    END AS years_since_pin_change,
    card_on_dark_web,
    mcc_description,
    TRY_CONVERT(decimal(18, 6), amount / NULLIF(credit_limit, 0)) AS amount_to_credit_limit,
    TRY_CONVERT(decimal(18, 6), amount / NULLIF(yearly_income, 0)) AS amount_to_yearly_income,
    CASE
        WHEN account_open_year IS NULL OR account_open_month IS NULL
    ...
    CASE
        WHEN expiration_year IS NULL OR expiration_month IS NULL
