"""Carga reproducible de las cinco fuentes públicas de FraudeDB.

El script adapta la carga por lotes de ``src/Migracion_sql_all.py``. No crea
la base ni elimina información: requiere ejecutar primero
``sql/01_create_source_tables.sql`` y omite por defecto toda tabla que ya
contenga filas.
"""
...
def iter_label_rows(path: Path) -> Iterator[tuple[int, int]]:
    """Lee el objeto ``target`` sin cargar los 159 MB completos en memoria."""
    carry = ""
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except Exception as exc:  # salida controlada para CLI
